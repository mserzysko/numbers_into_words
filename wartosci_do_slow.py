def number_to_letter(number):
    ...
    if liczba[-5]:
        if liczba[-5]=='1':
            num_in_words.insert(0, nastki[-4])
        else:
    ...
    if liczba[-8]:
        if liczba[-8]=='1':
            num_in_words.insert(0, nastki[-7]+' tysięcy')
        else:
            num_in_words.insert(0, jednosci[-7]+' tysięcy')
            num_in_words.insert(0, dziesiatki)
    else:
        if liczba[-7]:
    ...
    if liczba[-11]:
        if liczba[-11] == '1':
            num_in_words.insert(0, nastki[-10] + ' milionów')
        else:
            num_in_words.insert(0, jednosci[-10] + ' milionów')
            num_in_words.insert(0, dziesiatki)
    else:
        if liczba[-10]:
